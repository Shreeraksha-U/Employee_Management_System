...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($name) && !empty($email) && empty($nameErr) && empty($emailErr) && empty($dobErr)) {
        $sql_select_query = "SELECT email FROM employee WHERE email = ? AND email != ?";
        ...
        if ($r->num_rows > 0) {
            ...
            $sql = "UPDATE employee SET name = ?, email = ?, dob = ?, gender = ? WHERE email = ?";
            $stmt = $conn->prepare($sql);
