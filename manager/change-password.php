    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        if(empty($_REQUEST["new_pass"])){
            ...
            if (!preg_match("/^(?=.*[A-Z])(?=.*[!@#$%^&*])(?=.{8,})/", $new_pass)) {
                $new_passErr = "<p style='color:red'>Password must be at least 8 characters long and contain at least one uppercase letter and one special character</p>";
                 $new_pass = "";
            }
        }
        ...
        if(!empty($old_pass) && !empty($new_pass) && !empty($confirm_pass) ){
            ...
            $check_old_pass = "SELECT password FROM manager WHERE email = '$_SESSION[email]' && password = '$old_pass' ";
            $result = mysqli_query($conn , $check_old_pass);
            if( mysqli_num_rows($result) > 0 ){
               
                if( $new_pass === $confirm_pass ){
                  
                    $change_pass_query = "UPDATE manager SET password = '$new_pass' WHERE email = '$_SESSION[email]' ";
