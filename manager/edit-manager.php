...
<?php  
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (!empty($name) && !empty($email) && !empty($pass) && !empty($salary) && !empty($dob)) {
            ...
            $sql_select_query = "SELECT email FROM manager WHERE email = '$email' AND id != $id";
            $r = mysqli_query($conn, $sql_select_query);

            if (mysqli_num_rows($r) > 0) {
                $emailErr = "<p style='color:red'> * Email Already Registered</p>";
            } else {
                $sql = "UPDATE manager SET name = '$name', email = '$email', password = '$pass', dob = '$dob', gender = '$gender', salary = '$salary' WHERE id = $id";
                $result = mysqli_query($conn, $sql);
                if ($result) {
                    echo "<script>
                    ...
                    <div class="form-input-content">
                        <div class="card login-form mb-0">
                            <div class="card-body pt-4 shadow">
                                <h4 class="text-center">Edit Manager profile</h4>
