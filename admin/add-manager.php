...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($name) && !empty($email) && !empty($pass) && !empty($salary) && empty($dobErr) && empty($passErr) && empty($genderErr)) {
        ...
        $sql_select_query = "SELECT email FROM manager WHERE email = '$email' ";
        $r = mysqli_query($conn , $sql_select_query);

        if (mysqli_num_rows($r) > 0) {
            $emailErr = "<p style='color:red'> * Email Already Registered</p>";
        } else {
            $sql = "INSERT INTO manager (name, email, password, dob, gender, salary) VALUES ('$name', '$email', '$pass', '$dob', '$gender', '$salary')";
            ...
            if ($result) {
                ...
                $(document).ready(function(){
                    ...
                    $('#modalHead').hide();
                    $('#linkBtn').attr('href', 'manage-manager.php');
                    $('#linkBtn').text('View Managers');
                    $('#addMsg').text('Manager Added Successfully!');
                ...
                <div class="form-input-content">
                    <div class="card login-form mb-0">
                        <div class="card-body pt-4 shadow">                       
                            <h4 class="text-center">Add New Manager</h4>
